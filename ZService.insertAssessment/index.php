...
$stepData = property_exists($ZData->entity, 'resultData') ?
        $ZData->entity->resultData : $ZData->entity;
$pdo->beginTransaction();
$insResult = 0;
try {
    foreach ($stepData->scores as $assessmentRecord) {
        $tmpRecord = new stdClass();
        $tmpRecord->patientId = $stepData->patientId;
        $tmpRecord->roleId = $stepData->roleId;
        $tmpRecord->currentScoreId = $stepData->currentScoreId;
        $tmpRecord->itemID = $assessmentRecord->item;
        $tmpRecord->score = $assessmentRecord->value;        
        $insResult += $con->insertAssessment($tmpRecord);
    }

    if ($insResult != count($stepData->scores)) {
        $pdo->rollBack();
    }
    $pdo->commit();
} catch (Exception $ex) {
